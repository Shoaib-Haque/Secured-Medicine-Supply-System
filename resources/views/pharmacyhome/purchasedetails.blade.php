...
<body>
  ...
  <div id="admininputdiv">
    ...
    &nbsp;&nbsp;&nbsp;<font color="blue">Time:</font> {{$purchase->time}} <br>
  	&nbsp;&nbsp;&nbsp;<font color="blue">Total Amount:</font> {{$purchase->total}} Tk <br>
  	&nbsp;&nbsp;&nbsp;<font color="blue">Discount:</font> {{$purchase->discount}} Tk <br>
    &nbsp;&nbsp;&nbsp;<font color="blue">Net Amount:</font> {{$purchase->netammount}} Tk <br>
  	<table id ="list" align="center" style="width: 90%">
        ...
        @foreach($items as $items)
            <tr>
                <td style="min-width: 280px; max-width: 280px;">{{substr($items->name, 0, 20)}}<sub>{{$items->mg}}</sub></td>
                <td>{{$items->tname}}</td>
                <td>{{$items->mg}}</td>
                <td>{{$items->mrp}}</td>
