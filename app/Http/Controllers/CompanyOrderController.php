...
class CompanyOrderController extends Controller
{
    public function orderdetails($orderid){
    	...
    	$items = DB::table('ordercarts')
	    	        ...
	    	        ->where('oid',$orderid)
	    	        ->select('ordercarts.*', 'medicines.name as mname' , 'medicines.id as mid' ,
                             'medicines.price as price', 'medicines.mg')
	    	        ->get();
    ...
    public function deliveredorderdetails($orderid){
        ...
        $items = DB::table('ordercarts')
                    ->join('medicines', 'medicines.id', '=', 'ordercarts.mid')
                    ->where('ordercarts.oid',$orderid)
                    ->select('ordercarts.*', 'medicines.name as mname' ,'medicines.price as price', 'medicines.mg')
                    ->get();
        $batchitems = DB::table('companyinventories')
