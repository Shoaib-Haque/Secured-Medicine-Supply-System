...
class PharmacyPurchaseController extends Controller
{
    public function purchase(Request $req){
        ...
        for($i=0; $i<count($req->q); $i++){
            ...
            {
                if(count(DB::table('pharmacyinventories')
        ...
        for($i=0; $i<count($req->q); $i++)
        {
            if($req->q[$i] != 0 && $req->q[$i] != null && $req->medicineid[$i] != "")
            {
                DB::table('purchaseitems')->insertGetId(
                    ['mid' => $req->medicineid[$i] , 'purchaseid' => $purchaseid, 
                     'quantity' => $req->q[$i], 'total' => $req->t[$i]]
                );
            }
        }
    ...
    public function purchasesearch(Request $request)
    {
        ...
        {
            if($v->mg != null){
                $results[]=['id'=>$v->mid, 'mrp'=>$v->mrp, 'unit'=>$v->unit, 
                        'value'=> substr($v->tname,0,3) .":". $v->name ."(".$v->mg.")" ];
            }
            else{
                $results[]=['id'=>$v->mid, 'mrp'=>$v->mrp, 'unit'=>$v->unit, 
                        'value'=> substr($v->tname,0,3) .":". $v->name ];
            }
        }
    ...
    public function makebill(Request $req){
        $val = "";
        if($req->search){
           $data = DB::table('medicines')
            ->join('prescriptionitems', 'medicines.id', '=', 'prescriptionitems.medicineid')
            ->join('types', 'medicines.typeid', '=', 'types.id')
            ->where('prescriptionitems.prescriptionid',$req->search)
            ...
            if($data){
                ...
                foreach($data as $value => $search){
                    if($search->mg != null){
                        $val = substr($search->tname,0,3) .":". $search->name ."(".$search->mg.")";
                    }
                    else{
                        $val = substr($search->tname,0,3) .":". $search->name;
                    }
                    echo '<tr> <td><input type="text" value="'.$val.'" readonly name="medicine[]" 
                                    id="cmn'.$i.'" style="width:125px;"></td>
                               <td style="display:none;"><input type="hide" name="medicineid[]" value="' . $search->id . '" 
                                    id="cmn'.$i.'id"></td>
                               <td><input type="number" value="' . $search->mrp . '" readonly name="pt[]" 
                                    id="cmn'.$i.'p" style="background-color: #f2f2f2; border :none; width:55px;"></td>
                               <td><input type="number" id="cq'.$i.'" name="q[]" min="0" style="width:55px;"></td>
                               <td><input type="number" readonly name="t[]" 
                                    id="ct'.$i.'" style="background-color: #f2f2f2; border :none; width:55px;"></td>
                               <td><input type="button" onclick="remove('.$i.')"
                                   style="background-color: red; color:white; width:65px;" value="Remove"></td>
                            <tr>';
                    $i++;
                }
            }
