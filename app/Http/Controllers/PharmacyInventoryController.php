...
class PharmacyInventoryController extends Controller
{
    ...
    public function inventoryupdate($oid){
    	...
    	foreach ($itemlist as $key => $value) {
    		...
    		if(count(DB::table('pharmacyinventories')->where('mid', $value->mid)->where('pid',session('Pharmacyid'))->first())>0){
    ...
    public function unitsearch(Request $request)
    {
        ...
        {
            if($v->mg != null){
                $results[]=['unit'=>$v->unit." Unit Left" ,'value'=>substr($v->tname,0,3) .":". $v->name ."(".$v->mg.")"];
            }
            else{
                $results[]=['unit'=>$v->unit." Unit Left" ,'value'=>substr($v->tname,0,3) .":". $v->name];
            }
        }
    ...
    public function itemsearch(Request $req){
        ...
        if($data){
            foreach($data as $value => $search){
                echo '<tr> <td style="min-width: 300px; max-width: 280px;">' . $search->mname . ' <sub>' . $search->mg . '</sub>
                           </td>
                           <td>' . $search->tname . '</td>
                           <td>' . $search->unit . '</td>
                           <td>' . $search->mrp . '</td>
