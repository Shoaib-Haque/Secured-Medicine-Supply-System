...
class CompanyInventoryController extends Controller
{
    public function index(){
    	$inventorylist = DB::table('companyinventories')
					...
					->select('companyinventories.*', 'medicines.name as mname', 'medicines.mg')
    				->get();
    ...
    public function updatesearch(Request $request)
    {
        ...
        {
            if($v->mg != null){
                $results[]=['id'=>$v->id, 'value'=>substr($v->tname,0,3) .":". $v->name ."(".$v->mg.")"];
            }
            else{
                $results[]=['id'=>$v->id, 'value'=>substr($v->tname,0,3) .":". $v->name];
            }
        }
    ...
    public function itemsearch(Request $req){
        ...
        if($data){
            foreach($data as $value => $search){
                echo '<tr> <td>' . $search->name . ' <sub>' . $search->mg . '</sub></td>
